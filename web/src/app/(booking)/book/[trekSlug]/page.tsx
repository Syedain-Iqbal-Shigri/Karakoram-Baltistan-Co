...
export default function BookingPage({params}: {params: {trekSlug: string}}) {
    ...
    return (
      <div className="text-center py-20">
        ...
        <p className="text-stone-400 text-xs mt-1">
          Available: {Object.keys(trekData).join(', ')}
        </p>
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    setForm(prev => ({...prev, [name]: name === 'numberOfPersons' ? parseInt(value) : value}))
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/book/${slug}/payment?bookingId=${data.bookingId}&advance=${advanceAmount}&total=${totalAmount}`)
    } catch {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <a href={`/treks/${slug}`} className="text-emerald-600 text-sm font-medium">
          ← Back to {trek.title}
        </a>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 mb-8">
        <div className="flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="text-right">
            ...
            <div className="text-2xl font-bold text-stone-900">${trek.pricePerPerson.toLocaleString()}</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input type="text" name="fullName" value={form.fullName} onChange={handleChange}
                ...
                className="w-full border border-stone-300 rounded-xl px-4 py-3 text-stone-900 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
            <div>
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-6">
          ...
          <textarea name="specialRequests" value={form.specialRequests} onChange={handleChange}
            ...
            className="w-full border border-stone-300 rounded-xl px-4 py-3 text-stone-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
        </div>
        ...
        <p className="text-center text-stone-400 text-sm pb-8">
          🔒 Secure booking · Free cancellation 30 days before departure
        </p>
      </div>
    </div>
  )
